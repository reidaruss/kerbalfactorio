...
**Backlog after the dawn sweep:** ~~PH-366 mintStation still emptyDesign (D-015 not uniform for Anchorage, PS-43 stale)~~ **CLOSED 2026-08-14, PH-380 (lane/mintstation): `mintStation` mints a real one-part `DockingPort` design, `promoteVessel` refuses the station by name (`isStation`) instead of by accident, PS-43 corrected. Port POSE still comes off `socket_dock`, unchanged and stated as the right call.** R-BODY-2 eleven body-scoped fields on in-page switch, GP-761 stone haul 43 m short, GP-762 beltcargo drill rotation, demolish.js red (smelter would not go down at the head), persist.js red, zerog Z4 vacuous-pass risk, stationwalk.js stale aim, terrain LOD scope (world-gen), forestfloor re-baseline row, miner LOD1 proxy, belt curve/cap sections, antenna dish pick, rubble_pile.glb + scanning receipts owed by art, smelter 6 coplanar, 10 foliage validate_glb, expected.json Windows reproducibility, R100 docking save via real reload pair, stale-branch-base dispatch note (verifier: dry-merge before verifying web-only branches cut before an ABI bump).
**MERGE PROCEDURE, corrected 2026-08-16 after it broke main.** The union resolver Admin uses on merge conflicts keeps both sides of a hunk and strips the markers. That is correct for the append-only ledger files it was written for (NUMBERS.md, ADMIN.md, the controller logs, where two lanes each add a row). IT IS WRONG FOR CODE, and it proved it: resolving `tools/blender/texgen.py` and `web/src/render/instancing/Surfaces.ts` that way produced a Python IndentationError, four `masonry` rows, three `concrete` rows, and a duplicated TypeScript union, and it was pushed before any gate ran. Main was broken for about two minutes. THE RULE NOW: the union resolver runs on `.md` files ONLY. A conflict in any code file stops the merge; Admin either reconstructs it by hand from each parent (take each side's version of the file, extract the branch delta against the merge base, apply it three-way) or routes the reconciliation to a lane, and either way the language's own parser plus the domain gate runs BEFORE the push, not after. Verifiers had already flagged this exact hazard on this exact file (hoisted function tails leaving `None` returns with no syntax error) and the flag was not applied to Admin's own tooling.

**TWO HARNESS ITEMS ROUTED 2026-08-16, both confirmed pre-existing by a verifier that reproduced them.** (a) `probes/avatar.js`'s `armour.ok` is false and UNSTABLE for a reason that has nothing to do with armour: it differences scene triangle counts across three samples, and the unmodified probe measured a bare-minus-removed delta of 4 on one run and 2048 on the next, with a background task chip spawning between them. A check that differences a global counter across time is measuring the whole session, not its subject; owner gameplay/probes. (b) `tools/smoke/boot.mjs` binds an ephemeral port and drew 6667, which Chrome refuses outright as ERR_UNSAFE_PORT, so a run can die for a reason unrelated to the build; it must skip Chrome's unsafe-port list. Owner build-tooling.

**ENCODING RULE FOR EVERY LANE AND FOR ADMIN, 2026-08-16, learned by breaking a file.** A PowerShell `Get-Content` / `Set-Content` round trip decodes UTF-8 as the system ANSI codepage and re-encodes it, so every non-ASCII character in the WHOLE FILE is silently mangled and a BOM is added, while the edit itself looks clean. It hit `docs/controllers/rendering.md`: 101 section signs and 144 arrows corrupted across 95 lines, and the diff read as 97 REMOVED lines from a register nobody had touched. The repair is not to un-mangle bytes: restore the pristine file with `git checkout`, then re-apply the edit with a UTF-8-aware editor. THE RULE: never round-trip a text file through PowerShell's default encoding; use the Edit tool or an explicitly UTF-8 read and write, and check the diff's DELETION count before committing, because a clean insertion that reports deletions is this bug. **Also routed: a pre-existing mojibake cluster survives in `docs/phase1/OVERNIGHT-LOG.md` from the repo-move commit 67b4857, owner build-tooling.**

**A VERIFIER CONFIRMED A COMMENT IT HAD NOT READ, 2026-08-16, and the lane caught it.** Verifying the station-light rewrite, a verifier reported that "both instruments and the shadowcast comment carry the exact wording required". The shadowcast comment did not: it had been silently reverted and still asserted the very mechanism that commit's own ledger struck. The lane found it, fixed it, and said so rather than accepting the pass. TWO RULES FROM THIS. (1) EVERY VERIFIER BRIEF MUST DEMAND QUOTED EVIDENCE for any claim about file contents: a verifier that writes "carries the required wording" without pasting the line has checked nothing, and a confirmation is worth less than no check because the next reader trusts it. Ask for the quote. (2) THE REVERT TRAP UNDERNEATH IT, now also in NUMBERS.md: a control arm that runs `git checkout <ref> -- <path>` DESTROYS uncommitted edits under that path, and the restoring `git checkout HEAD -- <path>` brings back a HEAD that predates them, so a lane's prose can end up saying "refuted" while its code says the opposite. Commit or stash before any such control arm, and RE-READ the files afterwards rather than trusting the restore.

**THE SCREENSHOT IS NOT THE MEASURED FRAME, 2026-08-16 (RN-2018, verified with the lines quoted).** `run.mjs` runs the probe's eval at line 603, then `settle(20)` at 607, then screenshots at 633, and `Loop.settle` advances RENDERED FRAMES of a running loop, so real simulation time passes in the gap. Only `report.eval`, the probe's own rectangles, predates the settle; `report.stats`, `world` and `scene` are read after it, so those describe the later instant too. **A probe-published number and the PNG saved beside it are different moments, and the screenshot is an illustration rather than evidence for the numbers next to it.** The mechanism is conditional on motion, which is the useful half: on a scene that holds still the two instants are the same image, so static-shot frames remain sound and what to distrust is any frame of a scenario that can contain hostiles, or with unfrozen wind or unsettled IBL. It was found because a live spider walked through a ruin comparison, moved two legs across the measured rectangle between arms, and changed the player's health between captures; the rectangle it contaminated turned out to be 80 per cent not-masonry. `run.mjs` is deliberately NOT changed, because settle cannot move ahead of a probe that drives the world itself. **Consequence for this session: several look judgements relayed to Reid rested on such frames, and the honest position is that the measurements stand and the pictures are illustrative.** artframe now clears enemies in its own freeze block for every shot and publishes the cheat receipt with the player's hp, so this specific contaminant is gone at the source.

**Consolidated backlog (2026-08-14 evening, the two rows above merged after the wave landed):** the evening wave closed mintStation (PH-380), the KeyW freeze (GP-790..796, NOT REPRODUCED with a tripwire probe left armed), demolish.js and persist.js (GP-770..775, merged 4903369), zerog Z4's vacuous pass, stationwalk's stale aim, and the antenna dish pick (GP-805..807). Still open after the 2026-08-15 night waves: ~~R-BODY-2 playability half~~ (CLOSED, CE-70..74: the regrow was a STREAMING defect, NodeMemory.inheritDepletion, merged 96ebce6), GP-761 stone haul 43 m short (timing; probe-reaim lane in flight), ~~GP-762 beltcargo corridor~~ (CLOSED, GP-850: the fixture's 9 m reach cap, fixture fixed, merged 6784707), Z4's underlying standAt frame-body boarding gap (CE-54's open question, Z4 honestly red until closed), stationreload.mjs re-aim (probe-reaim lane in flight), ~~Input.uiHeld refcount~~ (CLOSED, GP-820 owner tokens, merged 154ba3d), ~~expected.json Windows reproducibility~~ (CLOSED, BT-100..101: CRLF fixed at source, libm ULP documented), ~~R100 docking save via real reload pair~~ (CLOSED, GP-866 reload pair built and found the resume defect; PH-381 fixed it, 30 km to 0.302 m), ~~carrier.js rotor C1 tolerance red~~ (DELISTED, CE-85..86, a dead instrument's number; carrier.js joined the sweep), ~~props=0 scatter shadows~~ (RN-1650, closed, did not reproduce), ruin 632 coplanar (VM lane in flight), med-tier frame-time noise (harness), **walkfeel.js validity gate host-timing sensitive (never validates on the Windows desktop on any tree; joins the frame-time noise item)**, **vesselreload.mjs stale phase-1 assertion (expects 1 vessel record from before the station became permanent; fails identically on clean main; owner physics/gameplay)**, **the extractCmd-null audit (91 probes the sweep silently skips; owner build-tooling)**, ~~padflat.js calls nonexistent of.surfaceAtLatLon~~ (CLOSED, GP-865, retired to a live pointer), harness gate census (quiet-box serial + Windows leg for D3D-only probes), **cantilever.js negative-control re-aim flake (BT-130's verifier: 2 of 2 red on the desktop, "the deepest carried cell could not be re-aimed", while the lane saw green; probe-side, diff was comment-only, owner gameplay/probes)**, **machineports.js phaseB.inputFell real red (routed by BT-130, owner factory-sim)**, ~~moonsite.js standalone red~~ (CLOSED, GP-920: FS-101 clock residue), ~~pad.js socket-snap enclosed-gap residual~~ (CLOSED, GP-922..924: the probe's own aimAt guard, ring completes 36 of 36; one narrower residual remains, the second-pad overlap-refusal misresolve, GP-924), ~~balance.js outcrop patch-deduction gap~~ (WITHDRAWN, FS-117: the core citation was false, outcrops link their patches, the zeroes were ToolRequired refusals misread; R11 rules ore tool-gated by design), ~~basesnap.js supported-cell refusal~~ (CLOSED, GP-935..936: the support rule was right, the probe's platform lacked the cell; deck placed from the site frame), ~~pad.js second-pad overlap misresolve~~ (CLOSED, GP-937..938: overlapping() was right, the aim across the placed pad hit its own body; control re-aimed honestly), **NEW UX finding, owner gameplay: aiming a placement ghost across a tall placed structure resolves the target block from the structure's own body hit, so a player trying to place near or beyond a 28 m pad gets a misresolved cell from every vantage the pad occludes (GP-937's diagnosis; five vantages tried); worth a design look at ghost aim-through or an occlusion cue**, ~~cantilever.js negative-control re-aim flake~~ (CLOSED, GP-921: FS-101 clock residue, consistently green now), ~~machineports.js phaseB red~~ (CLOSED, FS-115: the one-unit hopper made the 27-unit-buffer window impossible; probe asserts conservation now), ~~Z4 standAt frame-body boarding gap~~ (CLOSED, CE-100..104: standAt never boards, standAboard is the door, Z4 rides with sub-nanometre local drift, GP-400 closed in passing), ~~BuildDrag targeting~~ (CLOSED, FS-129..133: precedence redesign, fallback extends but never bridges, behind-target refusal on the lattice; assembler chain bit-deterministic), **NEW, host-timing: TWO OF THE 42 CTEST SUITES ARE LOAD SENSITIVE and go red on a shared box (gameplay-design lane, 2026-08-16): factory_sim_tests gates ups at or above 60.0 and read 48.9 under load at test_factory_sim.cpp:287, discovery_tests gates worstFresh under 5000 us and read 22,659 at test_discovery.cpp:1335; both pass idle (0.58 s and 10.79 s) and both pass on pristine main, so a green ctest taken while lanes are running proves less than it looks. Joins the walkfeel and med-tier frame-time noise item; owner build-tooling.** **NEW from the clock-fix verifier (GP-1013), two items. (a) THE MECHANISM IS BINADE-STRUCTURED, NOT MONOTONE: the story that a bigger performance.now means coarser ULP means a lost tick is only locally true. Measured, 1024 to 2048 ms is 63 per cent short while 16384 to 32768 ms is only 4 per cent, and the observed flip at about 32125 ms is a run crossing the 32768 ms binade edge mid-flight. The lane's window straddled exactly that edge, so its evidence is sound and its generalisation is not; anyone reasoning about wall-clock float error here should reason per binade. (b) assembler.js now takes 730 s on this box and returns NO_OUTPUT under probeall's default 240 s cap, with numbers bit-identical to GP-950, so it is harness cost and not a sim change: any census run including assembler needs its timeout raised or it scores a false red. Also the inherited-tick spread is wider than recorded, 179 to 212 across the verifier's ten sessions against the lane's 83 to 139, so that range is not a bound.** forest dusk §2c shape targets (look decision owed by Reid), sky/dusk before/after pair + playtest impressions (Reid).

**2026-08-14 late evening: the instrument bundle (`lane/instruments`, GP-805 to GP-807) closed all three struck-through items above.** zerog Z4's vacuous-pass risk is fixed (`carrierG` checked against an independently fetched `of.gravity(deckR)`) but Z4 is left HONESTLY RED, reproduced on an unmodified checkout too: `standAt(..., {frame:'body'})` boards nobody, so the station (1879.2552 m/s) leaves the seated point 4,698 m behind over Z4's own 2.5 s settle, well outside the gravity volumes — CE-54's own open boarding/settle-duration question, not this lane's to close. stationwalk.js's stale aim is fixed (`of.standAboard()` replaces `install.standPos`) and, past it, a deeper root cause: `of.station()` (`DebugGameplay.ts`) was passing a literal tick `0` into `stateOf` instead of the live tick, freezing every probe's frame at the station's last stamp; fixed (`loop.tickIndex`), and `stationwalk.js`'s own file-level `P`/`A`/`u` snapshot (the exact class its own header already warned against) rebuilt to refresh live at every phase and inside P6's 24-heading sweep. Verified `ok: true` end to end. The antenna dish pick (owed since GP-619) is fixed by recentring/enlarging `Antennas.pick`'s sphere against the real shipped mesh (measured, not guessed) with a new driven probe, `probes/antennapick.js`, `pass:true, fails:[]`. Full detail: gameplay.md GP-805 to GP-807, NUMBERS.md's own row.
...
- ~~**NEW genuine red, owner core-engine:** carrier.js rotor row C1, perTickM 31.32092 vs r*w 31.320866, relative ~1.7e-6 vs 1e-6 tolerance, deterministic~~ **DELISTED 2026-08-14 night, CE-85 (lane/rotor-c1, verified): the number was CE-53's old red re-filed off a fork that never absorbed a4f396d (the atan2 instrument fix), whose in-file comment predicts 1.7240873e-6 verbatim as the OLD instrument's artifact. Live chord residual ~2e-12 to 3.6e-12 against the 1e-9 gate, 43/43 checks twice on D3D11, tolerance untouched. The REAL defect was the harness: carrier.js documented no invocation so probeall.mjs extractCmd returned null and the sweep never ran it (CE-86, fixed). ROUTED AUDIT owed to build-tooling: 91 other probes still return null from extractCmd, so "not on the red list" and "green" remain different claims for all of them.**
...
**TWO NOTES FROM THE BT-250 MERGE, 2026-08-18.** (a) On the merged main, the first `npm run check` reported check:roles FAIL alongside the known-red check:limits; check:roles then passed standalone seconds later and a full rerun was 6/7 with only check:limits red. One transient, cause unknown, observed once, immediately post-merge with a heavy probe lane running on the same box. Not actioned; if a second sighting lands, it becomes a lane with this note as the first data point. (b) Admin process slip worth its own rule: the merge command chained `git push` after the gate with `;`, so the push went out regardless of the gate's result and the transient above was discovered AFTER main was already pushed. THE RULE: the gate and the push are separate commands in separate tool calls; push only after reading the gate's output.

**ROUTED FROM THE RN-2030 VERIFICATION, 2026-08-19.** The bimodality lane's clamp ships, but the verifier refuted the mechanism's last step: the camera never moved (both observers clamp alpha to [0,1]); the DRAWN HULL extrapolated, because renderTick = tickIndex - 1 + alpha is fed unclamped to mounts.syncWatchersAt and CarrierFrame.poseAt. Two follow-ups allocated: CE-130 to CE-134 (stamp lastMs from the rAF timestamp; decide the renderTick-versus-observer clamping asymmetry) and BT-255 to BT-259 (run.mjs --out on the station shot photographs the forest, not the station; every published station png from --out is the wrong scene while eval.png is right).
