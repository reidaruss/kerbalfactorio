...
**In flight:** Release lane on VM (settled ABI-24 rebuild, dist + expected.json, serve refresh at orbitalfoundry.lan); local L6 reveal lane (lane/reveal, GP-533..545: scanning antenna, one-shot of_poi_near reveal, markers, objective predicate); local ruin placement lane (lane/ruin-place, WG-166..185: ruin.glb drawn at the SiteCatalog site with collision, garrison via the Enemies.spawnGarrison seam, spawnGarrisonDebug deleted).
