...
**Consolidated backlog (2026-08-14 evening, the two rows above merged after the wave landed):** the evening wave closed mintStation (PH-380), the KeyW freeze (GP-790..796, NOT REPRODUCED with a tripwire probe left armed), demolish.js and persist.js (GP-770..775, merged 4903369), zerog Z4's vacuous pass, stationwalk's stale aim, and the antenna dish pick (GP-805..807). Still open: R-BODY-2 playability half (core-engine: in-page body round trip regrows drained nodes), GP-761 stone haul 43 m short (timing), GP-762 beltcargo corridor legRoom 2.6 needed vs 2.0 measured (honest red stands), Z4's underlying standAt frame-body boarding gap (CE-54's open question, Z4 honestly red until closed), **NEW: stationreload.mjs re-aim (instruments verifier: already red on main, now red for a new reason since of.station() reports the live tick; its bit-exact "unmoved" assertions were written against the frozen semantics)**, **NEW: Input.uiHeld is one boolean with eight callers and no reference count (GP-795: pack-under-pause plus one Escape leaves the pack open while movement resumes; the harmful mirror is a stuck-muted walk axis, the exact reported KeyW shape)**, expected.json Windows reproducibility, R100 docking save via real reload pair, carrier.js rotor C1 tolerance red (core-engine), ~~props=0 scatter shadows~~ (RN-1650, closed, did not reproduce), ruin 632 coplanar, med-tier frame-time noise (harness), padflat.js calls nonexistent of.surfaceAtLatLon, harness gate census (quiet-box serial + Windows leg for D3D-only probes), forest dusk §2c shape targets (look decision owed by Reid), sky/dusk before/after pair + playtest impressions (Reid).
