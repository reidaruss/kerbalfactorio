...
| 14 | 57.5 m | 1.8 m | `nearScene`, 1:1 |
| 15 (`maxDepth`) | 28.8 m | **0.9 m** | `nearScene`, 1:1 |

The cutoff is a `Regime` output, not a constant: `nearDepthCutoff = 6` on the surface, sliding to `4` then `0` through ascent so that at orbit *all* terrain is in the far scene. A chunk crossing the cutoff is generated by the same oracle at the same `centerUniverse`, so the two representations are subpixel-identical at the crossing distance (≥ 15 km) and the swap is invisible. Depth 14 at 1.8 m spacing was chosen as the resolution the 1 m voxel layer needs to seam against; WG-186's depth 15 at 0.899 m clears that bar from the other side.

**WG-186 took `maxDepth` from 14 to 15**, so the measured cell under the feet is 0.899 m rather than 1.799 m and the heightfield is now FINER than the 1 m voxel layer instead of coarser. Priced on a real D3D11 boot over four interleaved repeats per arm: high tier p50 8.00 -> 9.40 ms against the 16.6 ms budget, +112,243 triangles (+15.1%), near chunks 169 -> 184, pool never exhausted. What that does and does not buy is in `Config.ts` beside the constant and in `world-gen.md`; the short version is that it resolves sub-1.8 m relief into the SHADING and does **not** remove silhouette stepping on steep ground, because `lodstep.js` measures the p90 crease on a mountain flank as scale-invariant (~5 degrees at every depth from 13 to 16).
...
Structurally the two already agree (§4.6). The residual is cosmetic: the heightfield is a smooth grid (~1.8 m when this was written, 0.899 m since WG-186, i.e. now finer than the voxel rather than coarser), the voxel walls are 1 m axis-aligned blocks, so the pit mouth shows a small ledge. Measured at WG-191 on the `voxelface` frame, the finer heightfield raises the box iqr 24.27 -> 37.40 and leaves the shard structure of the cut face unchanged: that structure is a separate, pre-existing defect and is NOT the LOD.
