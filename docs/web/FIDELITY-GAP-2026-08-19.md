...
- **A4. Sky system (1 lane).** Sky-coloured hemispheric ambient replacing the scalar ladder, a cloud layer (a textured dome is enough; SE's own clouds are simple), a sun disc, and the aerosol re-reference from the paused L1 brief folded in, so distance goes blue instead of white.
