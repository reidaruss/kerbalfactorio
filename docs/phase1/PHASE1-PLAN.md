...
- **UPDATE 2026-06-15 — BUILT & SLICE LOOP PROVEN.** The build phase ran: C-8/scrub cleanup done, UE 5.7 project builds green over the cores, all headless slice logic green (ctest 13/13), and the **end-to-end slice loop is proven headlessly** (`core/tests/test_slice_e2e.cpp`): mine deposit → factory produces → research a tech → fly Forge→Cinder (SOI switch) → off-world Cinderite gate → save/reload, all through the real domain cores. The capstone surfaced 4 integration gaps (the seams between "domains work alone" and "loop is seamless"); **ALL 4 are now CLOSED (2026-06-15) — the slice loop is a clean typed chain, the e2e workarounds removed and the asserts strengthened, 13/13 ctest still green:**
  - **GAP-1 — CLOSED.** Deposit id unified: `gameplay::DepositId` is now a shared `using = worldgen::DepositId` (the `uint64` `FDepositNode::Id`), and gameplay mines a `worldgen::FDepositNode` directly (`mineDeposit(FDepositNode&, …)`) — no truncating bridge, the id carries verbatim into the persistence depletion diff. (gameplay.md GP-17, world-gen.md WG-15.)
  - **GAP-2 — CLOSED (keystone).** `FactorySim` gained `uint64 producedCountOf(ItemId)` — a per-`outputItem` lifetime tally alongside the existing total (Σ == `producedCount()`), covering both the live and on-rails paths. Science is now a **distinct, queryable producible** (`producedCountOf(scienceId) > 0` asserted end-to-end). Additive; 12 other suites unchanged. (factory-sim.md FS-13.)
  - **GAP-3 — CLOSED.** Authored the science recipes as data in `research.h`: `CraftAutomationScience` (plate → AutomationScience) and the off-world `RefineCinderScience` (Cinderite → CinderScience), registered via the new `SliceRegistry::registerRecipe`. The factory crafts the off-world science with a real recipe (the GP-2 gate item). (gameplay.md GP-18.)
  - **GAP-4 — CLOSED.** Research unlocks persist: `SliceState` carries the unlocked-`TechId` id list (gameplay schema→2) and `ResearchState::restoreUnlocked()` RESTORES the set on reload (not re-derived). Unlocks survive save→reload, proven with an empty science pool. (persistence.md PS-11.)
