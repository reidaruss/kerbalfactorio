...
| D-020 | 2026-08-13 | **Art style refined: Space Engineers is the primary fidelity and style target** (functional industrial realism, PBR metal and machined surfaces, engineered forms), amending D-018's Satisfactory bar; Skyrim/Elden Ring stay the aim-high references. Reid also wants "a lot of the mechanics behind the world" from Space Engineers. Deformable voxel terrain is already core. **Reid selected ALL FOUR mechanics as in scope (2026-08-13): structural physics and damage, block/grid construction, pressurization and life support, physical resource logistics.** These are post-pre-alpha ambitions unless the scope pass finds cheap pre-alpha slices; the pre-alpha line is unchanged. A scoping lane owes the work-item graph: fit against /core (health system as the damage seed, voxel terrain, belts vs conveyors, vessel.h parts vs grids), sequencing, and what each costs. | Reid 2026-08-13: "I think the artstyle of space engineers is what i want to go for. like that level of fidelity and allot of the mechanics behind the world as well"; all four mechanics selected when asked | Accepted (art + mechanics ambition); scope banked docs/scope/SE-MECHANICS-SCOPE-2026-08-13.md | admin + rendering + physics + core-engine |
| D-021 | 2026-08-13 | **KTX2 encoder is `ktx2-encoder@0.6.0` as a devDependency**, chosen over native `basisu` because basisu is not byte-reproducible across Windows and Linux (codegen differences even single-threaded, and UASTC+RDO is non-deterministic run to run), which would silently end DW-5's byte-identical texture rebuild gate. The pure-wasm encoder is byte-identical on both hosts at ~3 percent size cost and reuses texgen's own PNG decoder. First consumer wires it when the A4 wave raises resolutions. | A2b lane's measurement 2026-08-13; determinism gate is the campaign's load-bearing property | Accepted | rendering + build-tooling |
