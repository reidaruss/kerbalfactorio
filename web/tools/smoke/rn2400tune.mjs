// RN-2400 tuning-loop helper: just the six poses the K sweep is judged
// against (the seam pair and the four aerial whole-frame figures), not the
// full pass-condition set. `rn2400sweep.mjs` is the committed instrument.
...
const shotBy = (s) => (s.startsWith('forestair') || s.startsWith('flyover')
  || s === 'vista' || s === 'vistanoon' ? 'surface' : 'walk');
...
for (const shot of ['vista', 'vistanoon', 'flyover', 'flyovernoon', 'forestair', 'forestairnoon']) {
  const e = once(shot);
  if (!e.valid) { console.log(shot.padEnd(14), 'INVALID', e.why ?? ''); continue; }
  const ex = e.extra ?? {};
  console.log([shot.padEnd(14), `hzBand.warm=${w(ex.hzBand?.warm)}`,
    `skyHz.warm=${w(ex.skyHz?.warm)}`, `world.warm=${w(e.world?.warm)}`].join(' '));
}
