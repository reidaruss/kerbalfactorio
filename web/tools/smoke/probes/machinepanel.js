// GP-57 to GP-60: LEFT-CLICKING A MACHINE OPENS ITS SCREEN, the screen shows
...
(async () => {
  ...
  await sleep(0.5);
  // THE WHOLE-PROBE BANNER LEDGER (GP-60). Taken here, before anything, because
  ...
  for (let t = 0; t < budget / 0.05 && of.game().machines[MI].state.outCount - out0 < UNITS; ++t) {
    ...
    prev = drawn;
    samples++;
    // GP-60: once the first unit is done the objective chain has no smelting
  ...
  // ======================================================================
  // 6. GP-60: NO ROAMING TOAST FROM ROUTINE PRODUCTION
  // ======================================================================
  const gEnd = of.game();
  ...
  // path still FIRES (so GP-60 moved the ingot cue rather than breaking the
  ...
  // The input cell is published against a seam (GP-58) rather than silently
