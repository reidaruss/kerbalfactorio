...
export interface SlotRow {
  ...
  count: number;
  icon?: string;
  /** The ItemId, so a click can put this item on the bar. 0 for an empty slot. */
  item?: number;
  /** True when this item can go in a hotbar slot (a machine or a part). */
  placeable?: boolean;
}
...
export interface RecipeRow {
  ...
  icon?: string;
  /** W11: the tech that would unlock this recipe, or '' when it is not gated.
   *  A LOCKED row is still SHOWN, deliberately: a tech tree the player cannot
...
export class InventoryPanel extends Modal {
  ...
  constructor(parent: HTMLElement, stack: ModalStack,
              ...
              /** Put a pack item on the selected hotbar slot. The "put things
               *  in a hotbar" half of GP-26's ask: the bar has nine slots and
               *  the game now has twelve placeable things, so the loadout has
               *  to be something a player can change without a debug call. */
              private readonly onAssign: (item: number) => void = () => {}) {
    super('pack', stack);
    ...
    // One delegated listener, same argument as the craft column's: the tiles
    // are rebuilt whenever the pack changes and per-tile listeners leak.
    this.pack.addEventListener('click', (e) => {
      const t = (e.target as HTMLElement).closest('.of-slot[data-item]');
      if (t === null) return;
      const item = Number(t.getAttribute('data-item'));
      if (Number.isFinite(item) && item > 0) this.onAssign(item);
    });
  ...
  render(slots: SlotRow[], recipes: RecipeRow[]): void {
    ...
    if (packKey !== this.lastPack) {
      ...
      this.pack.innerHTML = slots.map((s) => (s.count > 0
        ? `<div class="of-slot filled${s.placeable === true ? ' place' : ''}"`
          + (s.placeable === true ? ` data-item="${s.item ?? 0}"` : '')
          + (s.placeable === true ? ' title="click to put on the hotbar"' : '')
          + `>${iconTag(s.icon, 'ico')}`
