...
#     big lobe after the fit into SNOW by the analytic ring-step; measured off
#     the exported bytes by the same method as the 33.8 above (mean inclination
#     of the ground-touching side faces, base n-gon excluded) the rim is 9.1.
...
# under half the 0.10 gap between the first two rings. That inequality alone
# does NOT prove ordering (_wob is multiplicative on both rings: ring0 spans
# [0.940, 1.060] and ring1 [0.846, 0.954], a 0.114 worst-case bound against a
# 0.10 gap); at these seeds it is MEASURED safe instead: no first-band face
# flares outward, min radial-normal component +0.036 (RN-2700 verifier). A
# seed change here must re-measure that.
