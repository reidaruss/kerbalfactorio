...
def build_lod0(root):
    ...
    # output on -Y at its outlet height.
    _mouth(mb, "Y", 1, IN_Z, 1.60, 0.70, 0.55, 0.40, 0.40, "Hazard")
    _mouth(mb, "X", 1, IN_Z, 1.60, 0.70, 0.55, 0.40, 0.40, "Hazard")
    _mouth(mb, "Y", -1, OUT_Z, 1.60, 0.50, 0.50, 0.35, 0.20, "Accent")
    ...
    # roof deck railing, then the plant
    for s in (-1, 1):
        mb.box((DECK, 0.10, 0.44), (0, s * 3.65, 3.53), "SteelDark")
        mb.box((0.10, DECK, 0.44), (s * 3.65, 0, 3.53), "SteelDark")
    _plant(mb)
    ...
    for sx in (-1, 1):
        for sy in (-1, 1):
            mb.box((0.70, 0.70, 2.95), (sx * 3.40, sy * 3.40, 1.775),
                   "SteelDark")
    mb.box((COLLAR + 0.06, COLLAR + 0.06, 0.10), (0, 0, 3.05), "Accent")
    _mouth_block(mb, "Y", 1, IN_Z, 1.60, 0.70, 0.55, 0.40, 0.40)
    _mouth_block(mb, "X", 1, IN_Z, 1.60, 0.70, 0.55, 0.40, 0.40)
    _mouth_block(mb, "Y", -1, OUT_Z, 1.60, 0.50, 0.50, 0.35, 0.20)
    _plant(mb)
    mb.box((3.60, 0.05, 0.20), (0, -3.785, STATUS_Z), "EmissiveState")
