...
def full_lod0():
    ...
    p.add(v, f, sm, roles)
    v, f, sm, roles = tc.canopy_mass(1.35, 1.25, 1.55, (-0.78, 0.30, 4.15),
                                     seg=13, seed=SEED + 1, jit=0.15,
                                     rings=(0.16, 0.40, 0.65, 0.86),
                                     radii=(0.55, 0.92, 1.00, 0.72),
                                     bands=("LeafDeep", "LeafDeep", "Leaf",
                                            "Leaf", "LeafLight"))
    p.add(v, f, sm, roles)
    v, f, sm, roles = tc.canopy_mass(1.25, 1.35, 1.45, (0.72, -0.38, 3.90),
                                     seg=13, seed=SEED + 2, jit=0.15,
                                     rings=(0.16, 0.40, 0.65, 0.86),
                                     radii=(0.55, 0.92, 1.00, 0.72),
                                     bands=("LeafDeep", "LeafDeep", "Leaf",
                                            "Leaf", "LeafLight"))
    p.add(v, f, sm, roles)
    v, f, sm, roles = tc.canopy_mass(0.95, 0.90, 1.10, (0.14, 0.78, 3.40),
                                     seg=11, seed=SEED + 3, jit=0.15,
                                     bands=("LeafDeep", "LeafDeep", "Leaf",
                                            "LeafLight"))
    p.add(v, f, sm, roles)
    # Fourth, smaller mass overlapping the first: two branch forks carrying
    # separate masses read as "a tree branched"; three clean non-overlapping
    # lobes read as "three balloons tied to a stick". This one breaks that.
    v, f, sm, roles = tc.canopy_mass(0.62, 0.58, 0.74, (-0.28, 0.68, 3.10),
                                     seg=9, seed=SEED + 4, jit=0.17,
